...
class AIVideoCreatorService:
    ...
    # ============================================
    # 旧的硬编码规则已迁移至 transform_rules.py
    # 以下方法保留用于向后兼容，但不再推荐使用
    # ============================================
    
    def _create_zoom_transform(
        self,
        duration_ms: float,
        start_scale: float,
        end_scale: float,
        ...
        """
        [DEPRECATED] 请使用 transform_rules.generate_transform()
        创建缩放动画
        """
        position_x = (DEFAULT_CENTER_X - center_x) * (end_scale - 1) * POSITION_OFFSET_FACTOR
        position_y = (DEFAULT_CENTER_Y - center_y) * (end_scale - 1) * POSITION_OFFSET_FACTOR
    ...
    def _create_static_transform(self, duration_ms: float = 3000) -> Dict:
        """[DEPRECATED] 创建静止变换 - 仍需两个关键帧确保时间范围完整"""
        return {
